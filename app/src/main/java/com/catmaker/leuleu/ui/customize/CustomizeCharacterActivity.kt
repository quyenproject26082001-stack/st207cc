...
import com.catmaker.leuleu.core.extensions.dLog
import com.catmaker.leuleu.core.extensions.eLog
...
import com.catmaker.leuleu.core.extensions.logEvent
import com.catmaker.leuleu.core.extensions.setImageActionBar
import com.catmaker.leuleu.core.extensions.showInterAll
import com.catmaker.leuleu.core.extensions.startIntentLeftToRight
...
import com.catmaker.leuleu.data.model.custom.ItemNavCustomModel
import com.catmaker.leuleu.databinding.ActivityCustomizeBinding
import com.catmaker.leuleu.dialog.DialogType
import com.catmaker.leuleu.dialog.YesNoDialog
import com.catmaker.leuleu.ui.home.DataViewModel
...
import com.catmaker.leuleu.ui.add_character.AddCharacterActivity
...
class CustomizeCharacterActivity : BaseActivity<ActivityCustomizeBinding>() {
    ...
    override fun initActionBar() {
        binding.actionBar.apply {
            ...
            btnActionBarCenter.visible() // Hide reset center button
    ...
    /**
     * Refresh toàn bộ UI sau khi Undo/Redo
     * Reload tất cả images và update adapters
     */
    private fun refreshUIAfterUndoRedo() {
        // 1. Reload tất cả images trong canvas
        viewModel.pathSelectedList.forEachIndexed { index, path ->
            if (path.isNotEmpty()) {
        ...
        // 4. Update color adapter
        colorLayerCustomizeAdapter.submitList(viewModel.colorItemNavList[viewModel.positionNavSelected].toList())

        // 5. Update color panel visibility
        checkStatusColor()

        // 6. Scroll color list to selected item
        if (viewModel.colorItemNavList[viewModel.positionNavSelected].isNotEmpty()) {
            val selectedIndex = viewModel.colorItemNavList[viewModel.positionNavSelected].indexOfFirst { it.isSelected }
            if (selectedIndex >= 0) {
                binding.rcvColor.smoothScrollToPosition(selectedIndex)
            }
        }
    }
    ...
    private fun handleReset() {
        ...
        dialog.onYesClick = {
            dialog.dismiss()
            lifecycleScope.launch(Dispatchers.IO) {
                // Save state BEFORE reset (so user can undo reset)
                viewModel.saveStateForUndo()
