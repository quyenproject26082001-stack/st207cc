package com.cat.emoji.fpf.maker.core.utils.key

object AssetsKey {
    const val ASSET_MANAGER = "file:///android_asset"
    const val DATA_ASSET = "file:///android_asset/data/"
    const val AVATAR_ASSET = "avatar"
    const val DATA = "data"
    const val DATA_CAT_MAKER = "data/data_cat_maker"
    const val DATA_CAT_MAKER_ASSET = "file:///android_asset/data/data_cat_maker/"
    const val DATA_EMOJI_MAKER = "data/data_emoji_maker"
    const val DATA_EMOJI_MAKER_ASSET = "file:///android_asset/data/data_emoji_maker/"
    const val BACKGROUND_ASSET = "background"
    const val SPEECH_ASSET = "speech"
    const val STICKER_ASSET = "sticker"
    const val RANDOM_LAYER = "RANDOM_LAYER"
    const val NONE_LAYER = "NONE_LAYER"
    const val SPLIT_LAYER = "-"

    const val FIRST_PNG = "1.png"
    const val FIRST_JPG = "1.jpg"
    const val FIRST_WEBP = "1.webp"

    const val NAVIGATION_IMAGE_PNG = "nav.png"
    const val NAVIGATION_IMAGE_JPG = "nav.jpg"
    const val NAVIGATION_IMAGE_WEBP = "nav.webp"


    const val AVATAR_CHARACTER_PNG = "avatar.png"
    const val AVATAR_CHARACTER_JPG = "avatar.jpg"
    const val AVATAR_CHARACTER_WEBP = "avatar.webp"

    val FIRST_IMAGE = arrayListOf(FIRST_PNG, FIRST_JPG, FIRST_WEBP)

}